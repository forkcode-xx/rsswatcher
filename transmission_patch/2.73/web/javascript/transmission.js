...
Transmission.prototype =
{
	...
	****/
	...
	initialize: function()
	{
		...
		$('#toolbar-rss').click($.proxy(this.openRssClicked,this));

		$('#prefs-button').click($.proxy(this.togglePrefsDialogClicked,this));
		$('#rss_save').click($.proxy(this.setRssList,this));
		$('#rss_remove').click($.proxy(this.delRssList,this));
		$('#rss-list').change($.proxy(this.showRssLink,this));
		...
		if (!isMobileDevice) {
			...
			//this.setupSearchBox(); //注销掉过滤输入框设置...
	...
	****/
	...
	togglePrefsDialogClicked: function(ev)
	{
		...
		if (e.hasClass('selected'))
			...
			this.prefsDialog.active(0);
			this.getRssList();
		}
	},
	
	openRssClicked: function(ev){
		this.prefsDialog.show();
		this.prefsDialog.active(4);
		this.getRssList();
	},
	
	getRssList: function(){
		this.sendRssRequest("method=get",$.proxy(function(rel){
			var opts = new Array();
			if(rel.length > 0){
				for(i in rel){
					if(rel[i].name != "" && rel[i].name != null){
						rel[i].url = this.urldecode(rel[i].url);
						opts.push("<option value=\""+rel[i].url+"\">"+rel[i].name+"</option>");
					}
				}
			}
			$('#rss-list').html(opts.join("\n"));
			$('#rss-url, #rss-name').val("");
		},this), "json");
	},
	showRssLink: function(){
		var name = $('#rss-list option:selected').text();
		var url = $('#rss-list option:selected').val();
		$('#rss-url').val(url);
		$('#rss-name').val(name);
	},
	...
	setRssList: function(){
		...
		if(rssurl != '' && rssurl != null){			
			if(!this.checkrsslink(rssurl)){
				alert("RSS URL is not correct, check the the PT site RSS download instructions!");
				return false;
			}
			if(this.urlExists(rssurl)){
				alert("RSS URL Exists!");
				return false;
			}
			if(rssname != "" && this.nameExists(rssname)){
				alert("RSS NAME Exists!");
				return false;
			}
			this.saveRssItem(rssurl);
		}else{
			alert("RSS URL can not be empty!"); return false;
		}
	},
	
	urlExists: function(url){
		var ret = false;
		var inlist = $('#rss-list').children("option");
		inlist.each(function(){
			if($(this).val() == url) ret = true;
		});
		return ret;
	},
	...
	nameExists: function(name){
		...
		var inlist = $('#rss-list').children("option");
		inlist.each(function(){
			if($(this).text() == name) ret = true;
		});
		return ret;
	},
	
	urlencode: function(url){
		return url.replace(/&/g,"%3D",url);
	},
	
	urldecode: function(url){
		return url.replace(/(%3D)/g,"&",url);
	},
	
	saveRssItem: function(url){
		var rssname = $('#rss-name').val();
		var urls = url.split(/\/+/);
		if($.trim(rssname) == '') rssname = urls[1]+"---"+this.getuid(url);
		this.sendRssRequest("method=set&name="+rssname+"&url="+this.urlencode(url),
		$.proxy(function(rel){ this.getRssList(); },this), 'html' );
	},
	
	delRssList: function(){
		var rsslist = $('#rss-list');
		var selected = rsslist.children("option:selected").val();
		if(selected != '' && selected != null){
			this.sendRssRequest("method=del&url="+this.urlencode(selected), $.proxy(function(rel){
				this.getRssList();
			},this), 'html' );
		}
	},
	
	sendRssRequest: function (data, callback, dataType){
		$.ajax({
			type:		"GET",
			url:		"../rss",
			dataType:	dataType,
	...
	checkrsslink: function(url){
		var re = /^(http:\/\/)[\w\.\/\?]+[\w=&(%3D)]+(uid|passkey=)\w+/;
		return re.test(url);
	},
	...
	***/
	...
	updateStatusbar: function()
	{
		...
		$('#speed-up-container').text( fmt.speedBps( u ) );
		$('#speed-dn-container').text( fmt.speedBps( d ) );
